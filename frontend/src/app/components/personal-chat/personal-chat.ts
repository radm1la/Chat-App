...
export class PersonalChatComponent implements OnInit, AfterViewChecked {
  ...
  initChat() {
    this.http.post<any>(`${environment.apiUrl}/personal/chat/${this.user.id}`, {}).subscribe(chat => {
      this.chatId = chat.id;
      this.loadMessages();
    });
  }
  ...
  loadMessages() {
    this.http.get<any>(`${environment.apiUrl}/personal/chat/${this.chatId}/messages?page=${this.currentPage}`).subscribe(res => {
